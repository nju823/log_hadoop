...
public class JobInitModel {
    protected String[] inPaths;//程序的输入目录
    protected String outPath;//程序的输出目录
    protected Configuration conf;//配置信息
    protected Job job;//job相关设置,如分布式文件缓存共享等
    protected String jobName;//该job的名称
    protected Class<?> jarClass;//mr的驱动程序类
    protected Class<? extends InputFormat> inputFormatClass;//输入格式化类
    protected Class<? extends Mapper> mapper;//mapper的实现类
    protected Class<?> mapOutKeyClass;//mapper输出的key类型
    protected Class<?> mapOutValueClass;//mapper输出的value类型
    protected Class<? extends Reducer> combinerClass;
    protected Class<? extends Partitioner> partitionerClass;
    protected Class<? extends Reducer> reducer;//reducer的实现类
    protected Class<?> reduceOutKeyClass;//reduce输出的key类型
    protected Class<?> reduceOutValueClass;//reduce输出的value类型
